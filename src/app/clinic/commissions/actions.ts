...
export async function saveCommissionRule(
  ...
  if (!context)
    return failure(
      "Apenas proprietários e administradores podem alterar regras de comissão.",
    );
  ...
  if (!uuidPattern.test(professionalProfileId))
    return failure("Selecione um profissional válido.");
  if (!["treatment_completed", "installment_paid"].includes(triggerEvent))
    return failure("Selecione um evento válido.");
  if (!["percentage", "fixed_amount"].includes(calculationType))
    return failure("Selecione uma forma de cálculo válida.");
  const validBasis =
    triggerEvent === "installment_paid"
      ? ["installment_gross", "payment_net"].includes(calculationBasis)
      : ["procedure_gross", "procedure_net_after_discount"].includes(
          calculationBasis,
        );
  if (!validBasis)
    return failure("Selecione uma base de cálculo válida para o evento.");
  if (!["general", "treatment", "category", "plan"].includes(scope))
    return failure("Selecione um escopo válido.");
  if (
    ...
    return failure(
      "Informe um percentual entre 0,01% e 100% ou um valor fixo maior que zero.",
    );
  if (
    !/^\d{4}-\d{2}-\d{2}$/.test(effectiveFrom) ||
    (effectiveUntil && !/^\d{4}-\d{2}-\d{2}$/.test(effectiveUntil))
  )
    return failure("Informe uma vigência válida.");
  if (effectiveUntil && effectiveUntil < effectiveFrom)
    return failure("A data final não pode ser anterior à data inicial.");
  if (calculationType === "fixed_amount" && scope !== "treatment")
    return failure(
      "Comissão em valor fixo precisa ser vinculada a um tratamento específico.",
    );
  if (triggerEvent === "installment_paid" && calculationType !== "percentage")
    return failure(
      "Comissões por recebimento devem ser percentuais para acompanhar pagamentos parciais e estornos.",
    );
  ...
  const { data: membership } = await context.supabase
    ...
    .maybeSingle();
  if (!membership)
    return failure("O profissional não possui vínculo ativo com esta clínica.");
  ...
  if (scope === "treatment") {
    ...
    if (!uuidPattern.test(treatmentPublicId))
      return failure("Selecione um tratamento específico.");
    const { data: treatment } = await context.supabase
      .from("treatments_catalog")
      .select("id")
      .eq("clinic_id", context.clinic.id)
      .eq("public_id", treatmentPublicId)
      .is("deleted_at", null)
      .maybeSingle();
  ...
  if (scope === "category") {
    ...
    if (treatmentCategory.length < 2 || treatmentCategory.length > 80)
      return failure("Informe uma categoria válida.");
  }

  if (scope === "plan") {
    const dentalPlanPublicId = readText(formData, "dentalPlanPublicId");
    if (!uuidPattern.test(dentalPlanPublicId))
      return failure("Selecione um convênio ou plano.");
    const { data: plan } = await context.supabase
      .from("dental_plans")
      .select("id")
      .eq("clinic_id", context.clinic.id)
      .eq("public_id", dentalPlanPublicId)
      .is("deleted_at", null)
      .maybeSingle();
    if (!plan)
      return failure("Convênio ou plano não encontrado nesta clínica.");
    dentalPlanId = plan.id;
  }

  const payload = {
    professional_profile_id: professionalProfileId,
    trigger_event: triggerEvent as "treatment_completed" | "installment_paid",
    calculation_type: calculationType as "percentage" | "fixed_amount",
    calculation_basis: calculationBasis as
      | "procedure_gross"
  ...
  if (publicId) {
    ...
    if (error)
      return failure(
        "Não foi possível atualizar a regra. Revise os dados e tente novamente.",
      );
  } else {
    const { error } = await context.supabase
      .from("commission_rules")
      .insert({ ...payload, clinic_id: context.clinic.id });
    if (error)
      return failure(
        "Não foi possível criar a regra. Revise os dados e tente novamente.",
      );
  }
